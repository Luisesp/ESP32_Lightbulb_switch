...
#define BTH_GPIO GPIO_NUM_0
static const uint32_t kOutputPins[] = {LED_GPIO};
static const uint32_t kInputPins[] = {BTH_GPIO};
static const uint32_t kOutputPinCount = sizeof(kOutputPins)/sizeof(uint32_t);
static const uint32_t kInputPinCount = sizeof(kInputPins)/sizeof(uint32_t);
...
bool SetLightStatus(bool newlightBulbStatus);
...
static void updateHAPCurrentState(void* _Nullable context, size_t contextSize)
{
    ...
    HAPAccessoryServerRaiseEvent(accessoryConfiguration.server, &lightBulbOnCharacteristic, &lightBulbService, AppGetAccessoryInfo());
}
//----------------------------------------------------------------------------------------------------------------------
...
HAPError HandleLightBulbOnWrite(
    ...
    if (SetLightStatus(value)) {
        //  moved to SetLightStatus ---> accessoryConfiguration.state.lightBulbOn = value;
        SaveAccessoryState();
...
//*******************
void interrupt_callbackhandler(int arg)
{
    printf("interrupt_callbackhandler -> GPIO[%d] \n", arg );
    if(arg == kInputPins[0])
    {
...
void AppCreate(HAPAccessoryServerRef* server, HAPPlatformKeyValueStoreRef keyValueStore) {
    ...
    // configure Hardware
    // Configure GPIO pins.
    accessoryConfiguration.device.lightbulbPin = kOutputPins[0];
    accessoryConfiguration.device.SwitchPin = kInputPins[0];

    HwConfigOutput((uint32_t*)kOutputPins,kOutputPinCount);
    HwConfigInterrupts((uint32_t*)kInputPins,kInputPinCount, interrupt_callbackhandler);
